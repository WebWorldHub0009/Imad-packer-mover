...
import bgImage from "../assets/home/abg.jpg"; // Premium moving-related bg

const faqCategories = {
  "Moving Basics": {
    icon: <FaTruckMoving className="text-[#3CB371] text-xl" />,
    faqs: [
      {
  ...
  "Services & Coverage": {
    icon: <FaMapMarkedAlt className="text-[#3CB371] text-xl" />,
    faqs: [
      {
  ...
  "Process & Pricing": {
    icon: <FaBoxOpen className="text-[#3CB371] text-xl" />,
    faqs: [
      {
...
const FAQ = () => {
  ...
  return (
    <section
      className="relative w-full py-20 px-4 md:px-10 bg-cover bg-center overflow-hidden"
    ...
    >
      ...
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-10 left-10 w-72 h-72 bg-[#3CB371]/20 blur-3xl rounded-full animate-pulse"></div>
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-[#C62828]/20 blur-3xl rounded-full animate-pulse"></div>
        <div className="absolute top-1/2 left-1/3 w-64 h-64 bg-[#3CB371]/20 blur-2xl rounded-full animate-pulse"></div>
      </div>
            ...
            {Object.entries(faqCategories).map(([category, data], idx) => (
              <motion.button
                ...
                className={`text-left w-full px-5 py-4 rounded-xl flex items-center gap-3 font-semibold text-sm md:text-base transition-all duration-300 shadow-sm ${
                  activeTab === category
                    ? "bg-[#3CB371] text-black shadow-lg"
              ...
              >
                {faqCategories[activeTab].faqs.map((faq, index) => (
                  <motion.div
                    key={index}
                    layout
                    className="border-b border-gray-200/20 py-4 cursor-pointer rounded-xl hover:bg-white/10 px-4 transition"
                  >
                    <button
                    ...
                    >
                      ...
                      <motion.span
                        ...
                        className="text-[#C62828] text-2xl font-bold transition-all duration-300"
                      >
                        +
                      </motion.span>
                    </button>
